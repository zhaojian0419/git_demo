...
File:test_05_generateRepayments.py
============================
    ...
    def test(self, items):
        ...
        if items.check_sql:
            if items.interface == 'generateRepayments':
                result1 = self.mysql.find_result(rp.replace_data(items.check_sql))  # 请求前数据表中记录的个数
        ...
        try:
            ...
            if items.check_sql:  # 如果需要数据库校验
                if items.interface == 'bidloan':
                    investId = self.mysql.select(rp.replace_data(items.check_sql))[0][0] # 竞标得到investId
